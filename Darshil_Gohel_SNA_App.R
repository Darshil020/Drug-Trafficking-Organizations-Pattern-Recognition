...
server <- function(input, output) {
  dataInput11 <- reactive({
    ...
    df=read.table('https://raw.githubusercontent.com/Darshil020/Drug-Trafficking-Organizations-Pattern-Recognition/master/COCAINE_DEALING.csv',header=TRUE,sep=',')
    head(df)
    rownames(df) <- df[,1]
    df <- df[ -c(1)];
    return(df)
